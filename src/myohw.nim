...
type
  MyohwServices* {.pure, size: sizeof(cint).} = enum ## \
    ...
    ControlService = 0x00000001, ## Myo info service.
    ImuDataService = 0x00000002, ## IMU service.
    ClassifierService = 0x00000003, ## Classifier event service.
    EmgDataService = 0x00000005, ## Raw EMG data service.
    MyoInfoCharacteristic = 0x00000101, ## Serial number for this Myo and
      ## various parameters which are specific to this firmware. Read-only
      ## attribute. See `MyohwFwInfo <#MyohwFwInfo>`_.
    classifierEventCharacteristic = 0x00000103, ## Classifier event data.
      ## Indicate-only characteristic. See `MyohwPose <#MyohwPose>`_.
    emgData0Characteristic = 0x00000105, ## Raw EMG data. Notify-only
      ## characteristic.
    firmwareVersionCharacteristic = 0x00000201, ## Current firmware version.
      ## Read-only characteristic. See `MyohwFwVersion <#MyohwFwVersion>`_.
    emgData1Characteristic = 0x00000205, ## Raw EMG data. Notify-only
      ## characteristic.
    emgData2Characteristic = 0x00000305, ## Raw EMG data. Notify-only
      ## characteristic.
    commandCharacteristic = 0x00000401, ## Issue commands to the Myo. Write-only
      ## characteristic. See MyohwCommand.
    imuDataCharacteristic = 0x00000402, ## See MyohwImuData. Notify-only
      ## characteristic.
    emgData3Characteristic = 0x00000405, ## Raw EMG data. Notify-only
      ## characteristic.
    motionEventCharacteristic = 0x00000A02 ## Motion event data. Indicate-only
      ## characteristic.


  MyohwStandardServices* {.pure, size: sizeof(cint).} = enum ## \
    ## Standard Bluetooth device services.
    batteryService = 0x0000180F, ## Battery service
    deviceName = 0x00002A00,  ## Device name data. Read/write characteristic.
    batteryLevelCharacteristic = 0x00002A19 ## Current battery level
      ## information. Read/notify characteristic.
...
type
  MyohwCommand* {.pure, size: sizeof(cuchar).} = enum ## \
    ## Kinds of commands.
    setMode = 0x00000001, ## Set EMG and IMU modes.
      ## See `MyohwCommandSetMode <#MyohwCommandSetMode>`_.
    setModeCustom = 0x00000002, ## Set EMG and IMU modes with custom parameters.
      ## See `MyohwCommandSetModeCustom <#MyohwCommandSetModeCustom>`_.
    vibrate = 0x00000003, ## Vibrate.
      ## See `MyohwCommandVibrate <#MyohwCommandVibrate>`_.
    deepSleep = 0x00000004, ## Put Myo into deep sleep.
      ## See `MyohwCommandDeepSleep <#MyohwCommandDeepSleep>`_.
    vibrate2 = 0x00000007, ## Extended vibrate.
      ## See `MyohwCommandVibrate2 <#MyohwCommandVibrate2>`_.
    setSleepMode = 0x00000009, ## Set sleep mode.
      ## See `MyohwCommandSetSleepMode <#MyohwCommandSetSleepMode>`_.
    unlock = 0x0000000A, ## Unlock Myo.
      ## See `MyohwCommandUnlock <#MyohwCommandUnlock>`_.
    userAction = 0x0000000B ## Notify user that an action has been recognized /
...
type
  MyohwCommandHeader* = object
    ...
    command*: MyohwCommand ## Command to send.
    payloadSize*: cuchar ## Number of bytes in payload.
...
type
  ...
  MyohwImuMode* {.pure, size: sizeof(cuchar).} = enum ## \
    ## IMU modes.
    none = 0x00000000, ## Do not send IMU data or events.
    sendData = 0x00000001, ## Send IMU data streams (accelerometer, gyroscope,
      ## and orientation).
    sendEvents = 0x00000002, ## Send motion events detected by the IMU
      ## (e.g. taps).
    sendAll = 0x00000003, ## Send both IMU data streams and motion events.
    sendRaw = 0x00000004 ## Send raw IMU data streams.


  MyohwClassifierMode* {.pure, size: sizeof(cuchar).} = enum ## \
    ## Classifier modes.
    disabled = 0x00000000, ## Disable and reset the internal state of the
      ## onboard classifier.
    enabled = 0x00000001 ## Send classifier events (poses and arm events).
...
type
  MyohwCommandSetMode* = object
    ...
    header*: MyohwCommandHeader ## `command` =
      ## `MyohwCommand.setMode <#MyohwCommand>`_, `payloadSize` = 3.
    emgMode*: MyohwEmgMode ## EMG sensor mode.
    imuMode*: MyohwImuMode ## IMU mode.
    classifierMode*: MyohwClassifierMode ## Classifier mode.


type
  MyohwVibrationType* {.pure, size: sizeof(cuchar).} = enum ## \
    ## Kinds of vibrations.
    none = 0x00000000, ## Do not vibrate.
    short = 0x00000001, ## Vibrate for a short amount of time.
    medium = 0x00000002, ## Vibrate for a medium amount of time.
    long = 0x00000003 ## Vibrate for a long amount of time.
...
type
  MyohwCommandVibrate* = object
    ## Vibration command.
    header*: MyohwCommandHeader ## `command` =
      ## `MyohwCommand.vibrate <#MyohwCommand>`_, payload_size = 1.
    vibrationType*: MyohwVibrationType ## Vibration type.
...
type
  ...
  MyohwCommandVibrate2* = object
    ...
    header*: MyohwCommandHeader ## `command` =
      ## `MyohwCommand.vibrate2 <#MyohwCommand>`_, payload_size = 18.
...
type
  MyohwSleepMode* {.pure, size: sizeof(cuchar).} = enum ## \
    ## Sleep modes.
    normal = 0, ## Normal sleep mode. Myo will sleep after a period of
      ## inactivity.
...
type
  MyohwCommandSetSleepMode* = object
    ...
    sleepMode*: MyohwSleepMode ## Sleep mode.


type
  MyohwUnlockType* {.pure, size: sizeof(cuchar).} = enum ## \
    ## Unlock types.
    lock = 0x00000000, ## Re-lock immediately.
    unlockTimed = 0x00000001, ## Unlock now and re-lock after a fixed timeout.
    unlockHold = 0x00000002 ## Unlock now and remain unlocked until a lock
      ## command is received.
...
type
  MyohwCommandUnlock* = object
    ...
    header*: MyohwCommandHeader ## `command` =
      ## `MyohwCommand.unlock <#MyohwCommand>`_, payload_size = 1.
    unlockType*: MyohwUnlockType ## Unlock type.
...
type
  MyohwCommandUserAction* = object
    ## User action command.
    header*: MyohwCommandHeader ## `command` =
      ## `MyohwCommand.userAction <#MyohwCommand>`_, payload_size = 1.
    actionType*: MyohwUserActionType ## Type of user action that occurred.
...
type
  ...
  MyohwImuData* = object
    ...
    orientation*: MyohwImuDataOrientation ## Orientation data, represented as a
      ## unit quaternion. Values are multiplied by myohwOrientationScale.
    accelerometer*: array[3, cshort] ## Accelerometer data. In units of g.
      ## Range of + -16. Values are multiplied by
      ## `myohwAccelerometerScale <#myohwAccelerometerScale>`_.
    gyroscope*: array[3, cshort] ## Gyroscope data. In units of deg/s.
      ## Range of + -2000. Values are multiplied by
      ## `myohwGyroscopeScale <#myohwGyroscopeScale>`_.
...
type
  ...
  MyohwMotionEvent* = object
    ...
    data*: MyohwMotionEventData ## Event-specific data.
...
type
  MyohwClassifierEventArmData* = object
    ...
    arm*: MyohwArm ## The arm that generated the event data.
    xDirection*: MyohwXDirection ## X-direction relative to user's arm.
